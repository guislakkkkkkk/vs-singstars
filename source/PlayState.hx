...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		//if (cpuControlled && CoolUtil.isRecording()) spook = true;
		...
		switch (curStage)
		{
			case 'stage': //Week 1
				...
				var stageStuff:BGSprite = new BGSprite('stagestuff', -64 * 2, (600 / 1.8), 0.9, 0.9);
				stageStuff.setGraphicSize(Std.int(stageStuff.width * 1.1));
				stageStuff.updateHitbox();
				add(stageStuff);
				...
				if(!ClientPrefs.lowQuality) {
					...
					stageCrowd = new BGSprite('stagecrowd', -750, 300, 1.3, 1.3, ['stage trolls']);
					stageCrowd.setGraphicSize(Std.int(stageCrowd.width * 0.9));
				}
		...
		precacheList.set('missnoteg1', 'sound');
		precacheList.set('missnoteg2', 'sound');
		precacheList.set('missnote3', 'sound');
	...
	var spookyTimer:FlxTimer;
	public function spookyVid() 
	{
		spookyTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
			}, 5);
	...
	public function startVideo(name:String)
	{
		...
		//video.canSkip = false; // WTF
		video.finishCallback = function()
		{
			if (name == 'liquidknows') {
			Sys.exit(0);
			} else if (name == 'liquidwins') {
	...
	public function startCountdown():Void
	{
		...
		if (spook) startVideo('liquidknows');

		inCutscene = false;
		var ret:Dynamic = callOnLuas('onStartCountdown', [], false);
		if(ret != FunkinLua.Function_Stop || spook) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				switch (curStage) {
					case 'mall':
						...
						santa.dance(true);
					case 'stage':
						if(!ClientPrefs.lowQuality && tmr.loopsLeft % 4 == 0) stageCrowd.dance(false);
				}
	...
	{
		if (!ClientPrefs.noRatings) scoreTxt.text = 'Score: ' + songScore
		+ ' | Misses: ' + songMisses
		+ ' | Rating: ' + ratingName
		+ (ratingName != '?' ? ' (${Highscore.floorDecimal(ratingPercent * 100, 2)}%) - $ratingFC' : ''); else scoreTxt.text = 'Score: ' + songScore
		+ ' | Misses: ' + songMisses;
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				{
					var liqNext = (Paths.formatToSongPath(SONG.song) == "virgin-rage");
					if (liqNext)
					{
						...
						new FlxTimer().start(2.5, function(tmr:FlxTimer) {
							cancelMusicFadeTween();
							startVideo('liquidwins');
						});
						new FlxTimer().start(38.1, function(tmr:FlxTimer) {
	...
	function noteMissPress(direction:Int = 1):Void //You pressed a key when there was no notes to press for this key
	{
		...
		if (!boyfriend.stunned)
		{
			...
			totalPlayed++;
			RecalculateRating(true);
			if (boyfriend.curCharacter == 'solid-guitar') FlxG.sound.play(Paths.soundRandom('missnoteg', 1, 3), FlxG.random.float(0.5, 0.7)); else FlxG.sound.play(Paths.soundRandom('missnote', 1, 3), FlxG.random.float(0.1, 0.2));
			...
			if(boyfriend.hasMissAnimations && boyfriend.specialAnim != true) {
				boyfriend.playAnim(singAnimations[Std.int(Math.abs(direction))] + 'miss', true);
			}
			if (boyfriend.specialAnim != true) vocals.volume = 0;
		}
