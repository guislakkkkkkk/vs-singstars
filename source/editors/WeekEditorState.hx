...
class WeekEditorState extends MusicBeatState
{
	...
	function addWeekUI() {
		...
		tab_group.add(new FlxText(displayNameInputText.x, displayNameInputText.y - 18, 0, 'Description:'));
	...
	function reloadAllShit() {
		...
		displayNameInputText.text = weekFile.description;
	...
	function updateText()
	{
		...
		txtWeekTitle.text = weekFile.description.toUpperCase();
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>) {
		if(id == FlxUIInputText.CHANGE_EVENT && (sender is FlxUIInputText)) {
			if(sender == weekFileInputText) {
				...
				weekFile.description = displayNameInputText.text.trim();
				updateText();
