...
class MenuCharacterEditorState extends MusicBeatState
{
	...
	function addCharacterUI() {
		...
		offsetXStepper = new FlxUINumericStepper(140, idleInputText.y, 10, characterFile.offsetX, 0.1, 30, 2);
		offsetYStepper = new FlxUINumericStepper(offsetXStepper.x + 60, offsetXStepper.y, 10, characterFile.offsetY, 0.1, 30, 2);
		...
		tab_group.add(offsetXStepper);
		tab_group.add(offsetYStepper);
		...
		if(fullPath != null) {
			...
			if(rawJson != null) {
				...
				if(loadedChar.idle_anim != null && loadedChar.confirm_anim != null) //Make sure it's really a character
				{
					...
					offsetXStepper.value = characterFile.scale;
					offsetYStepper.value = characterFile.scale;
					updateOffset();
					_file = null;
					return;
				}
